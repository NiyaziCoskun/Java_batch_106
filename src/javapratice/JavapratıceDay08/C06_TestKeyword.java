...
*/
  ...
  public static void testsonucu(char[][] answers, char[] keys) {
      ...
      for (int i = 0; i < answers.length ; i++) {//her bir ögrenci
          int sayac=0;//her bır ögrencınıdoğru cevabını sayacagı için sayac buraya konur

          for (int j = 0; j < answers[i].length ; j++) {//herr ögrencıının cevaplarına j cevaplara bakar
              if (answers[i][j]==keys[j]){//cevap anahtarı ıle ögrencı cevapları karşışaltırdık
                  sayac++;
          ...
          System.out.println(i+1  + " nolu ogrencının dogru cevap sayisi:" + sayac);
      }
